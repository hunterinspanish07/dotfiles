...
4. **Only if that marker is present and the pane is idle again** does it paste the message. It then polls until the composer shows *this* paste (collapse chrome that was not on screen before, or a message line that was not on screen before — the launcher adds a unique id line so PREFIX/SUFFIX already in a compacted transcript are not the only evidence) before submitting. If the paste never lands, that is a MISFIRE: no Enter, and the still-live session is told so. Compact can show its marker while still running — if still busy after the settle bound, wait until idle and tell the session (do not paste).

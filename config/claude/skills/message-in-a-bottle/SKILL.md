...
3. Reads the pane back and checks for a harness-specific marker that only a reset which actually **ran** leaves (visible chrome only — not scrollback, not the echoed command text, not bare nouns that fit in transcript prose):
   - `/clear`: Claude Code → startup version banner; OpenCode → empty-session splash prompt with no live session header (`/clear` is an alias of `/new`). Harness is `opencode*` / `*claude*` from tmux `pane_current_command`; anything else is unknown and misfires (never borrows the other harness's proof).
...
4. **Only if that marker is present and the pane is idle again** does it paste the message (unbracketed — bracketed paste collapses the body so the unique id line is not observable). It then polls until the composer shows *this* paste — a multiset increase in collapse chips (count of `[Pasted ~N lines]` rose; same-size re-bottles share the chip string so presence alone is not proof), or a message line that was not on screen before (the launcher adds a unique id line so PREFIX/SUFFIX already in a compacted transcript are not the only evidence) — before submitting. If the paste never lands, that is a MISFIRE: no Enter; drain until that same paste evidence is gone from the pane (pane-observed, not a fixed clear chord), then tell the still-live session — if evidence will not clear, log only and do not type. Busy proof is harness-gated live chrome (Claude `(Ns `; OpenCode status-line `esc interrupt` + token stats), never a bare phrase that fits in transcript. Compact can show its marker while still running — if still busy after the settle bound, wait until idle and tell the session (do not paste).

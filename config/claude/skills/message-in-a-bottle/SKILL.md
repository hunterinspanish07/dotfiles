...
3. Reads the pane back and checks for a harness-specific marker that only a reset which actually **ran** leaves (visible chrome only — not scrollback, not the echoed command text, not bare nouns that fit in transcript prose):
   ...
   - `/compact`: Claude Code → `Compacting conversation` / `Conversation compacted`; OpenCode → the full-width box-drawing compaction banner (the middot status line is leftover from prior compacts and is not the gate).
...
4. **Only if that marker is present and the pane is idle again** does it paste the message. It then polls until the composer shows *this* paste (OpenCode collapse chrome, or a message line that was not on screen before) before submitting — a whole-pane redraw is not proof. If the paste never lands, that is a MISFIRE: no Enter, and the still-live session is told so. Compact can show its marker while still running — if still busy after the settle bound, that is a MISFIRE (no paste), not a race.
